...
def main(n_server, staleness_threshold, eval_interval=15, eval_pool_size=1, training_duration=1800, log_path=None, yappi_log_path=None, f1_epoch_log_path=None):
    global epochs
...
#        server_list=[]
#        server_list=np.append(server_list, {"hook": hook, "host": "10.138.0.13", "verbose": True})
#        server_list=np.append(server_list, {"hook": hook, "host": "10.138.0.15", "verbose": True})
        servers = []
#        for i in range(n_server):
        while True:
            try:
                server=WebsocketClientWorker(id="dataserver-1",port="2222",hook=hook,host="10.138.0.13")
#                    server = WebsocketClientWorker(id=f"dataserver-1", port=2222, **server_list[i])
        ...
        while True:
            try:
                server=WebsocketClientWorker(id="dataserver-2",port="2222",hook=hook,host="10.138.0.15")
                servers.append(server)
                epochs[server.id]=0
                break
            except (ConnectionRefusedError, timeout):
        ...
        while True:
            try:
                server=WebsocketClientWorker(id="dataserver-3",port="2222",hook=hook,host="10.138.0.16")
                servers.append(server)
                epochs[server.id]=0
                break
            except (ConnectionRefusedError, timeout):
...
def train(server, n_server):
    ...
    print(server.id)
    if (server.id=="dataserver-1"):
        delay=30*random.random()
        print("delay= ",delay)
        time.sleep(delay)
        print("send to server-1")

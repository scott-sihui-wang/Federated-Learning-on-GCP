...
from flwr.server.strategy import FedAvg,FedPAQ, Strategy
...
#    def fit(self, num_rounds: int, sync_level=1: int) -> History:
...
#        for current_round in range(1, num_rounds/sync_interval + 1):
        for current_round in range(1, r1+1):
            # Train model and replace previous global model
#            res_fit = self.fit_round(rnd=current_round,sync_lvl=sync_interval)
            if r2>0 and current_round == r1:
                self.strategy.sync_level = r2
    ...
    def fit_round(
        ...
        client_instructions = self.strategy.configure_fit(
            rnd=rnd, parameters=self.parameters, client_manager=self._client_manager#, interval=sync_lvl
        )
        if not client_instructions:
